...
dc1394_cameracapture camera0;
dc1394_cameracapture camera1;//TODO
...
(JNIEnv *env, jobject obj) {
	jbyteArray jb;
	jb = env->NewByteArray (camera0.frame_height * camera0.frame_width * 2);
	if (dc1394_dma_single_capture (&camera0) != DC1394_SUCCESS) {
		printf ("Error at dc1394_dma_single_capture\n");
	}
	env->SetByteArrayRegion (jb, 0, camera0.frame_height * camera0.frame_width * bytesPerPixel, (jbyte*)camera0.capture_buffer);
	dc1394_dma_done_with_buffer(&camera0);
	return (jb);
}
...
JNIEXPORT void JNICALL Java_br_ufabc_bassani_jmf_firewire_media_protocol_sfwc_StereoFireWireCamera_cleanUp  (JNIEnv *env, jobject obj) {
	dc1394_dma_unlisten(handle, &camera0);
	dc1394_dma_release_camera(handle, &camera0);
...
JNIEXPORT jboolean JNICALL Java_br_ufabc_bassani_jmf_firewire_media_protocol_sfwc_StereoFireWireCamera_start (JNIEnv *env, jobject obj) {
	if (dc1394_start_iso_transmission(handle, camera0.node) != DC1394_SUCCESS)
	{
...
JNIEXPORT jboolean JNICALL Java_br_ufabc_bassani_jmf_firewire_media_protocol_sfwc_StereoFireWireCamera_stop (JNIEnv *env, jobject obj) {
	if (dc1394_stop_iso_transmission (handle, camera0.node) != DC1394_SUCCESS) {
