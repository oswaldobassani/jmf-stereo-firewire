...
public class LiveStream implements PushBufferStream, Runnable, CaptureDevice {
	...
	protected RealStereoFireWireCamera sfwc = new RealStereoFireWireCamera();
	protected boolean connected = false;

	protected Format[] formats = new Format[1];

	protected float frameRate = RealStereoFireWireCamera.getFrameRate();
	private int width = RealStereoFireWireCamera.getWidth();
	private int height = RealStereoFireWireCamera.getHeight();
	...
	public void setupDeviceCaptureInfo () {
		...
		formats[0] = RealStereoFireWireCamera.getFormat();

		cdi = new CaptureDeviceInfo ("PointGrey Dragonfly", new MediaLocator ("rsfwc://"), formats);
	}
	...
	public LiveStream() {
		...
		size = new Dimension(width, height);
		maxDataLength = size.width * size.height;// *3;
		rgbFormat = (RGBFormat)RealStereoFireWireCamera.getFormat();
